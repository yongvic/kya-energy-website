...
boutonMicro.addEventListener('click', async () => {
    ...
    if (!isRecording) {
        try {
            ...
            addMessage("Impossible d'accéder au microphone. Veuillez vérifier les permissions.", 'error');
        }
    } else {
        mediaRecorder.stop();
        isRecording = false;
...
async function transcribeAudio(audioBlob) {
    ...
    try {
        ...
        while (true) {
            const pollingResponse = await fetch(pollingEndpoint, { headers: { 'authorization': assemblyAIApiKey } });
            ...
            if (transcriptionResult.status === "completed") {
                ...
                throw new Error(`La transcription a échoué: ${transcriptionResult.error}`);
            }
...
const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        if (mutation.attributeName === 'class' && !chat.classList.contains('hidden')) {
